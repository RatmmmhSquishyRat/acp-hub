...
| on-disk | `~/.grok/sessions/<enc-cwd>/<uuid>/` (`chat_history.jsonl` + `summary.json`) | adapter read-only replay | `grok -r <id> -p --permission-mode dontAsk --no-plan` plus explicit edit/shell/MCP denies |
