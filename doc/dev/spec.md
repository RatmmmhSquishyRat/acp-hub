...
- `crate::hub` 的公共路径保持稳定：`CoreHub`、`HubClient` 和已有公共 DTO 继续从该路径导出。
- `crates/hub/src/hub.rs` 只负责私有子模块声明和公共 re-export，不再承载具体业务实现。
- Hub 生产代码按类型、状态、注册表、对话、prompt、生命周期、RPC dispatch 和 client 分离；测试按共享 fixture、注册表、client、operation 和 replay 分离。
- Hub 范围内单个 Rust 文件不能达到 1,000 行；实现时以不超过约 900 行作为预留边界。接近边界时，实现者需要主动形成拆分设计并执行，不等待用户提醒。
- 拆分属于内部维护边界调整，不能改变 ACP、JSON-RPC、CLI、MCP、存储语义或 `crate::hub::*` 的公共 API。
