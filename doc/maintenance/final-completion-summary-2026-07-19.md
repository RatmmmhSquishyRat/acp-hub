...
- `session/new` 在 request 前建立 per-agent bounded quarantine，并在同一
  connection generation 内完成 owner-aware publication 或 rollback；matching、
  existing-bound、unknown updates 的发布、重放与丢弃边界均有回归测试；
...
- proxy ACK 改为物理 leg 的 token、canonical semantic identity 与 bytes
  三元记账，不再依赖逻辑 FIFO；
- daemon discovery 在旧 singleton 持锁者退出后会竞争已释放的 lock、清理
  stale state 并启动 replacement，不再只轮询失效 metadata 直到 15 秒超时；
...
| 完整 Rust tests | 239 passed，0 failed，5 ignored |
| Cursor fixture | 28 passed，1 个 live-vendor mutation skip |
| Grok fixture | 44 passed，1 个 live-vendor mutation skip |
| dependency policy | `cargo deny --all-features check` pass；仅保留配置允许的 duplicate/unmatched-allowance warning |
| packaged public consumer | pass；外部 consumer 对 exact ACP SDK 1.2.0 编译通过 |
...
### 11.4 发布前 Git checkpoint

本次最终本地矩阵是在分支 `codex/resolve-review-feedback` 的已提交基线
`12a7fc2d95cf785acb238e35cb9ccaf312eb2cab` 及其维护中 worktree 上执行；
当时 `origin/main` 为
`af859b8dccbb9664f3917c3ae4219ea1e1d75125`。该 checkpoint 只记录发布前
候选来源，不替代后续 commit、PR、tag、release 与 clean-worktree 现场。
发布结果需要按 GitHub 与 crates.io 的实际状态追加记录。

## 12. PR re-review 后的最终代码闭合

PR #29 对较早 commit 的第二轮自动 review 又发现五个真实问题，因此 section 11
的“无剩余 finding”只能视为当时 checkpoint。本轮重新复现、修复并独立复审了：

- Grok shutdown 现在等待 prompt/delete/upstream 子进程树，使用有界 TERM/KILL
  fallback；shutdown 开始后不再接纳新请求，prompt 私有目录在 managed cleanup
  后才移除；
- 成功删除 live Grok session 后建立进程生命周期 tombstone，后续 load/prompt
  不再落回同一 upstream，迟到 update 被过滤；
- daemon notification receiver lag 不再静默跳过，连接会关闭并要求重连同步；
- cancel 先取得 exact persisted run/conversation cancelling ownership，再发送
  ACP notification，并与 prompt terminalization 串行；通知失败先恢复持久层，
  rollback 失败则保留 cancelling fail-closed 状态和原始错误；
- unbind/revoke 先摘除 terminal active ownership，在锁外执行三次有界 cleanup；
  即使最终失败也不会永久占用 quota/activity，两个真实进程测试覆盖 fallback。

完整 finding 见 Review Book section 12，任务状态见 Task Plan 的 T-611/T-612。
当前代码闭合不等于发布闭合；最终测试数、hosted CI、merge、tag、crates.io 和
GitHub Release 只能在对应动作真实完成后记录。
...
| 完整 Rust tests | 244 passed，0 failed，5 ignored fixtures |
| Cursor / Grok synthetic adapters | Cursor 28 passed + 1 intentional skip；Grok 47 passed + 1 intentional skip |
| dependency / package | `cargo deny` pass；exact packaged ACP 1.2.0 consumer pass；CLI package list pass |
| privacy / hygiene | 当前用户名与真实本机路径无匹配；secret pattern 无匹配；全部 Rust 文件低于 900 行；`git diff --check` pass |
| independent re-review | daemon/cancel、terminal、Grok 三个 lane 均 `APPROVED` |
...
build-script 临时 executable。截图所列三个被隔离生成物路径均已不存在，本次
没有关闭杀软或建立永久排除项，也不再用该本地路径反复触发 heuristic；同一
reviewed source 的隔离 Ubuntu Rust 1.91 hosted job 已成功，作为 MSRV 权威
证据。Avira UI 仍可能保留历史隔离记录，但它不表示当前仓库内仍存在被拦截
文件。
