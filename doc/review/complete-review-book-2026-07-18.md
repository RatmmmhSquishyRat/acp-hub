...
| R-SDK-002 | Medium | The root manifest used caret requirements, so the packaged core alone could later resolve another compatible ACP 1.x release even though the review and consumer fixture claimed exact 1.2.0 identity. | Both public ACP dependencies use `=1.2.0`. PowerShell and Bash package checks inspect the normalized packaged manifest and fail unless both exact requirements survive packaging. |
...
| R-CONV-001 | Medium | A successful remote `session/new` could commit the local conversation row and then fail snapshot or pending-update publication, returning no conversation id while leaving an orphaned local projection that blocked a safe retry. Pre-response notifications could also be attributed to a duplicate or failed creator. | A bounded per-agent quarantine begins before the request and shares the ordinary pending quotas. The current connection generation and session identity remain owned through publication or cleanup. Matching notifications publish only after row and snapshot creation; owner conflicts and capture failures discard them. Non-matching updates for an existing binding are replayed, while unknown updates are discarded when no session id is returned. Rollback removes only this operation's matching binding, runtime state, and parent row and preserves both the original and cleanup errors. |
...
| R-PRIV-001 | High | Ordinary endpoint list/inspect DTOs cloned `fs.allowed_roots`, exposing absolute local authorization paths even though transport secrets were redacted. | Public capability DTOs retain only the operational boolean flags. Filesystem roots and the `allowed_roots` field are absent from serialized ordinary reads and covered by real private-path markers. |
...
| R-FLOW-002 | Medium | Stdio input accumulated an incomplete line outside `partial_bytes`, allowing retained complete frames plus another near-32 MiB partial frame to exceed the aggregate flow ceiling. | Each consumed stdio chunk is progressively reserved; newline and CRLF bytes count toward the exact wire limit; only newline-terminated frames transfer atomically to a physical reservation; EOF, parse, budget, and drop paths release partial bytes. |
| R-DAEMON-001 | High | When daemon discovery observed the singleton lock as busy and that owner exited before metadata connection, the client polled stale state for the complete startup timeout instead of taking over the released lock. | Contending clients now poll both metadata and singleton-lock ownership. After the prior owner exits, one client removes stale state, spawns the replacement daemon, and connects within the remaining original timeout. |
| R-DAEMON-002 | High | Unix socket hardening unconditionally requested atomic socket-mode selection, but Interprocess reports that operation as unsupported on macOS. The detached child exited before writing metadata, leaving every first macOS CLI command to time out after 15 seconds. | The parent directory is hardened to `0700` before bind. Platforms that support atomic socket mode retain it; an `Unsupported` result retries inside that owner-only directory and immediately enforces `0600` on the bound socket. Opt-in inherited startup stderr makes future detached-child failures observable. |
| R-DAEMON-003 | High | A 0.2 client reused any reachable resident daemon without a compatibility check. Against 0.1, a side-effecting prompt could execute before the client discovered the old response schema and invited a duplicate retry. | A side-effect-free `hub/daemon/handshake` with an independent exact RPC contract version runs before the client is exposed. Missing, malformed, or mismatched handshakes return `DaemonUnavailable` before any business RPC. |
...
| R-MCP-001 | Medium | Tampered, filter-mismatched, or stale message cursors fell through to MCP internal error even though callers can recover by discarding the cursor and restarting pagination. | `InvalidCursor` and `StaleCursor` map to MCP `INVALID_PARAMS` with typed `reason` data and stale-generation details. |
| R-CAPTURE-001 | Medium | Repeated load/resume capture operations reused a session budget, so individually bounded refreshes accumulated until later refreshes failed. | Every validated capture-operation boundary replaces the session budget with a fresh zeroed budget; stale connections cannot reset the active generation. |
...
| R-GROK-002 | Medium | `grok sessions delete` children were outside shutdown ownership and could continue mutating vendor state after the adapter exited. | Deletion children are session-keyed, live/headless prompts and deletion are mutually exclusive per session, Unix children use a dedicated process group, and shutdown terminates every tracked deletion tree alongside prompt trees. |
...
- 239 Rust tests, with 5 fixture-dependent tests deliberately ignored;
- Cursor: 28 fixture checks passed, with 1 live-vendor mutation skipped;
- Grok: 44 fixture checks passed, with 1 live-vendor mutation skipped;
...
### 11.3 Explicit external boundaries at this pre-publication review checkpoint
